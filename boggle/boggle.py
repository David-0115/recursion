...
def find(board, word, l=0, lst=None, idx=None, visited=None):
    """Can word be found in board?"""

    if visited is None:
        visited = set()

    if l == 0 and lst is None and idx is None:
        for new_lst in range(len(board)):
            for new_idx in range(len(board[0])):
                if find(board, word, 0, new_lst, new_idx, set()):
                    return True
        return False
    ...
    if lst < 0 or lst >= len(board) or idx < 0 or idx >= len(board[0]) or (lst, idx) in visited:
        return False

    if board[lst][idx] != word[l]:
        return False
    ...
    for row, col in directions:
        new_lst, new_idx = lst+row, idx+col
        if find(board, word, l+1, new_lst, new_idx, visited):
            return True

    visited.remove((lst, idx))
    return False
